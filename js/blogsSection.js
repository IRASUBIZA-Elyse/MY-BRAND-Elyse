...
fetch(sameBlogsUrl + "/api/blogs")
  ...
  .then((output) => {
    ...
    for (let i = 0; i < max; i++) {
      ...
      let titleResult = addThreeDotsAfterLength(title, allTitle);
      function addThreeDotsAfterLength(title, allTitle) {
        return title.length > allTitle
          ? title.slice(0, allTitle) + "..."
          : title;
      }
      blogshtml += `<div class="blog" key=${output[i]._id}>
  
  <div class="blogImg">
    <img src="${output[i].image}" alt="blog img" />
  </div>
  <div class="blog-title">
    <p class="blogTitle"><b>${titleResult}</b></p>
    <br />
  </div>
  ...
  <span class="text_primary singlePage" key=${output[i]._id}>read more<span>
  <div class="svgForBlogs">
    
    <div class="feedback likes"><p class="likesNumber">${output[i].likes}</p><svg xmlns="http://www.w3.org/2000/svg" width="26" height="22" viewBox="0 0 26 22" fill="none">
    ...
    const blogs = document.querySelectorAll(".singlePage");
    blogs.forEach((blg) => {
